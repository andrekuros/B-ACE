...
const DEFAULT_NUM_UAVS := "15"
const DEFAULT_NUM_TARGETS := "50"
...
func _set_num_uavs():
	...
	for i in range(num_uavs-1):
		var addPlane= env.get_node("Plane").duplicate()
		addPlane.add_to_group("AGENT")
		addPlane.position = Vector3(rng.randf_range(-200.0, 200.0),50,rng.randf_range(-350.0, -30.0))		
		addPlane.set_meta('id', i + 2)
		env.add_child(addPlane)	
